  ...
  module Images
    class MasterHashingService
      ...
      def init_children_status
        @children_count.times.each do |child_num|
          redis.setex(redis_child_key(child_num), 12.hours, 'init')
        end
      end

      def get_children_status
        @children_count.times.map do |child_num|
          redis.get redis_child_key(child_num)
        end
      end
      ...
      def reset_children
        @children_count.times.map do |child_num|
          redis.del redis_child_key(child_num)
        end
      end
