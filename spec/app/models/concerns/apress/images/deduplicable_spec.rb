      ...
      context 'when destroy fingerprint parent' do
        ...
        context 'when duplicate destroy enqueued' do
          ...
          it do
            ...
            expect(image1.subject_type).to eq 'ProductImage'
            expect(image1.img.exists?(:original)).to be_truthy
            expect(Resque.delayed?(Apress::Images::DeleteImageJob, image1.id, image1.class.name)).to be_truthy
          end
        end
      end
    ...
    describe DefaultDuplicatedImage, type: :model do
      ...
      context 'when create duplicate' do
        it do
          ...
          expect(Resque.delayed?(Apress::Images::DeleteImageJob, image1.id, image1.class.name)).to be_truthy
          expect(Resque.delayed?(Apress::Images::DeleteImageJob, image2.id, image2.class.name)).to be_truthy
      ...
      context 'when parent image reprocess after duplicate builded' do
        ...
        it do
          expect(Resque.delayed?(Apress::Images::UpdateDuplicateImageJob, image3.id, image3.class.name)).to be_truthy
        end
      end
      ...
      context 'when destroying duplicate' do
        ...
        it do
          ...
          expect(Resque.delayed?(Apress::Images::UpdateDuplicateImageJob, image3.id, image3.class.name)).to be_falsey
        end
      end
